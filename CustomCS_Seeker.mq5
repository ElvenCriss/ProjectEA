...
// Define body sizes
   ...
   bool currentCandleBigEnough = (body_current > body_prev);
   ...
   for(int i = 0; i < srCount; i++)
     {
      ...
      if( //(low_current <= srLevel + tolerance && high_current >= srLevel - tolerance) &&
         (isBullishEngulfing || isBearishEngulfing || isMorningStar || isEveningStar /*|| isDoji*/))
        {
         ...
         if ( isBullishEngulfing && isBullishEngulfing)
         {
         low_box = MathMin(low_current, low_prev); // Lowest low
         time_start = iTime(symbol, timeframe, shift + 1);
         }
         ...
         if(isBullishEngulfing)
            ObjectSetInteger(0, objName, OBJPROP_COLOR, clrLightGreen); // Box color
         else
            if(isBearishEngulfing)
               ObjectSetInteger(0, objName, OBJPROP_COLOR, clrPink); // Box color
            else
               if(isMorningStar)
                  ObjectSetInteger(0, objName, OBJPROP_COLOR, clrGreen); // Box color
               else
                  if(isEveningStar)
                     ObjectSetInteger(0, objName, OBJPROP_COLOR, clrCrimson); // Box color
